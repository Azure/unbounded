...
type PeerTransport int32

const (
	PeerTransport_PEER_TRANSPORT_TCP  PeerTransport = 0
	PeerTransport_PEER_TRANSPORT_RDMA PeerTransport = 1
)

// Enum value maps for PeerTransport.
var (
	PeerTransport_name = map[int32]string{
		0: "PEER_TRANSPORT_TCP",
		1: "PEER_TRANSPORT_RDMA",
	}
	PeerTransport_value = map[string]int32{
		"PEER_TRANSPORT_TCP":  0,
		"PEER_TRANSPORT_RDMA": 1,
	}
)

func (x PeerTransport) Enum() *PeerTransport {
	p := new(PeerTransport)
	*p = x
	return p
}

func (x PeerTransport) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (PeerTransport) Descriptor() protoreflect.EnumDescriptor {
	return file_config_proto_enumTypes[0].Descriptor()
}

func (PeerTransport) Type() protoreflect.EnumType {
	return &file_config_proto_enumTypes[0]
}

func (x PeerTransport) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use PeerTransport.Descriptor instead.
func (PeerTransport) EnumDescriptor() ([]byte, []int) {
...
type PeerSpec struct {
	...
	Transport     PeerTransport          `protobuf:"varint,2,opt,name=transport,proto3,enum=unbounded.storage.config.PeerTransport" json:"transport,omitempty"`
	Address       string                 `protobuf:"bytes,3,opt,name=address,proto3" json:"address,omitempty"`
	HcaNuma       *uint32                `protobuf:"varint,4,opt,name=hca_numa,json=hcaNuma,proto3,oneof" json:"hca_numa,omitempty"`
...
func (x *PeerSpec) GetTransport() PeerTransport {
	if x != nil {
		return x.Transport
	}
	return PeerTransport_PEER_TRANSPORT_TCP
}
...
const file_config_proto_rawDesc = "" +
	...
	"\fmax_inflight\x18\x05 \x01(\rR\vmaxInflight\"\xfb\x02\n" +
	"\vTopologyCfg\x12(\n" +
	...
	"\vnic_workers\x18\n" +
	" \x01(\x04R\n" +
	"nicWorkersJ\x04\b\x01\x10\x02J\x04\b\x02\x10\x03J\x04\b\x03\x10\x04R\x15rdma_progress_per_hcaR\x15rdma_handlers_per_hcaR\x14tcp_fallback_threads\"\xf0\x01\n" +
	"\x06P2pCfg\x12(\n" +
	...
	"\n" +
	"_successorB\x0e\n" +
	"\f_predecessor\"\xc0\x01\n" +
	"\bPeerSpec\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x04R\x02id\x12E\n" +
	"\ttransport\x18\x02 \x01(\x0e2'.unbounded.storage.config.PeerTransportR\ttransport\x12\x18\n" +
	"\aaddress\x18\x03 \x01(\tR\aaddress\x12\x1e\n" +
	"\bhca_numa\x18\x04 \x01(\rH\x00R\ahcaNuma\x88\x01\x01\x12\x16\n" +
	...
	"\rPeerTransport\x12\x16\n" +
	"\x12PEER_TRANSPORT_TCP\x10\x00\x12\x17\n" +
	"\x13PEER_TRANSPORT_RDMA\x10\x01*G\n" +
	"\bDiskKind\x12\x12\n" +
...
var file_config_proto_goTypes = []any{
	(PeerTransport)(0),   // 0: unbounded.storage.config.PeerTransport
	(DiskKind)(0),        // 1: unbounded.storage.config.DiskKind
	(BackendKind)(0),     // 2: unbounded.storage.config.BackendKind
	(FrontendKind)(0),    // 3: unbounded.storage.config.FrontendKind
	(*Config)(nil),       // 4: unbounded.storage.config.Config
	(*StartupCfg)(nil),   // 5: unbounded.storage.config.StartupCfg
	(*MetricsCfg)(nil),   // 6: unbounded.storage.config.MetricsCfg
	(*MemoryCfg)(nil),    // 7: unbounded.storage.config.MemoryCfg
	(*FabricCfg)(nil),    // 8: unbounded.storage.config.FabricCfg
	(*TopologyCfg)(nil),  // 9: unbounded.storage.config.TopologyCfg
	(*P2PCfg)(nil),       // 10: unbounded.storage.config.P2pCfg
...
var file_config_proto_depIdxs = []int32{
	10, // 0: unbounded.storage.config.Config.p2p:type_name -> unbounded.storage.config.P2pCfg
	...
	5,  // 5: unbounded.storage.config.Config.startup:type_name -> unbounded.storage.config.StartupCfg
	7,  // 6: unbounded.storage.config.StartupCfg.memory:type_name -> unbounded.storage.config.MemoryCfg
	8,  // 7: unbounded.storage.config.StartupCfg.fabric:type_name -> unbounded.storage.config.FabricCfg
	9,  // 8: unbounded.storage.config.StartupCfg.topology:type_name -> unbounded.storage.config.TopologyCfg
	6,  // 9: unbounded.storage.config.StartupCfg.metrics:type_name -> unbounded.storage.config.MetricsCfg
	11, // 10: unbounded.storage.config.P2pCfg.routing_plan:type_name -> unbounded.storage.config.RoutingPlan
	0,  // 11: unbounded.storage.config.PeerSpec.transport:type_name -> unbounded.storage.config.PeerTransport
	1,  // 12: unbounded.storage.config.DiskSpec.kind:type_name -> unbounded.storage.config.DiskKind
	2,  // 13: unbounded.storage.config.BackendSpec.kind:type_name -> unbounded.storage.config.BackendKind
	3,  // 14: unbounded.storage.config.FrontendSpec.kind:type_name -> unbounded.storage.config.FrontendKind
