...
// stepIdentity proves the operator turns nine anonymous nodes into
...
// stepShared proves the thing single-node access modes were hiding: one volume,
// one claim, two pods on two different nodes, both holding the device at the
// same time and both seeing the same bytes.
//
// This is the shape a DaemonSet wants. The claim is not a per-pod object, so a
// thousand pods against this volume would still be one PersistentVolume and one
// PersistentVolumeClaim, and the per-pod cost would be one NodePublishVolume:
// a stat and a bind mount, with nothing on the pod start path talking to the
// apiserver.
//
// Every read and write here uses O_DIRECT deliberately. The nodes share the
// media, not the page cache: each node has its own cache over its own
// /dev/ublkb<minor>, so a buffered read on the second node could answer from
// local memory and the test would prove nothing about racer.
func stepShared(t *testing.T, h *harness) {
	ctx := context.Background()

	// Relocate left a consumer on workers[3]. Both of these are in the
	// six-node zone, so neither pod is reaching across a gateway to read.
	first, second := h.workers[3], h.workers[0]

	held := h.createConsumer(t, ctx, second)
	h.waitForPod(t, ctx, 6*time.Minute, held)

	// Both pods are running now. Under the old access-mode check the second
	// NodeStageVolume would have been refused outright.
	for _, pod := range []string{consumerName(first), held} {
		want := h.digestOfPattern(t, ctx, pod)

		if got := h.digestOfDevice(t, ctx, pod); got != want {
			t.Fatalf("%s read %s, want %s while sharing the volume", pod, got, want)
		}
	}

	t.Logf("%s and %s hold the same volume and read the same bytes", first, second)

	// Now write from the second node and read it back from the first. The
	// extent is LWW, so the write lands unconditionally and the only question
	// is whether the first node observes it.
	marker := fmt.Sprintf("yes racer-e2e-shared-fedcba9876543210 | head -c %d > /tmp/shared", patternBytes)

	want, err := h.exec(ctx, held, marker+" && md5sum /tmp/shared | cut -d' ' -f1")
	if err != nil {
		t.Fatalf("digest the shared marker in %s: %v", held, err)
	}

	if _, err := h.exec(ctx, held, fmt.Sprintf(
		"dd if=/tmp/shared of=%s bs=1M count=%d oflag=direct conv=fsync status=none",
		consumerDevice, patternBytes>>20,
	)); err != nil {
		t.Fatalf("write the shared marker from %s: %v", second, err)
	}

	got := h.digestOfDevice(t, ctx, consumerName(first))
	if got != strings.TrimSpace(want) {
		t.Fatalf("%s read %s after %s wrote %s", first, got, second, strings.TrimSpace(want))
	}

	t.Logf("a write on %s is readable on %s through the same volume", second, first)

	// Put the original pattern back and drop to a single consumer, because the
	// migration step reads the pattern digest and expects to find one pod.
	if _, err := h.exec(ctx, held, fmt.Sprintf(
		"%s && dd if=/tmp/pattern of=%s bs=1M count=%d oflag=direct conv=fsync status=none",
		patternScript(), consumerDevice, patternBytes>>20,
	)); err != nil {
		t.Fatalf("restore the pattern from %s: %v", second, err)
	}

	h.deleteConsumerOn(t, ctx, second)
}
...
func stepShrink(t *testing.T, h *harness) {
	...
	for _, name := range retired {
		patch := fmt.Sprintf(`{"metadata":{"labels":{%q:"false"}}}`, racercomponent.EnrollmentLabel)

		node := &corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: name}}
		if err := h.cli.Patch(ctx, node, client.RawPatch(types.MergePatchType, []byte(patch))); err != nil {
			t.Fatalf("opt %s out: %v", name, err)
		}
	}
...
func (h *harness) createVolume(t *testing.T, ctx context.Context, class string) {
	...
	volume := &corev1.PersistentVolume{
		...
		Spec: corev1.PersistentVolumeSpec{
			...
			// ReadWriteMany, because a racer volume genuinely is exportable
			// from every node at once and the Shared step relies on it. The
			// volume is raw block, so there is no filesystem above the device
			// to be surprised by a second writer; coherence is the consumer's
			// problem and racer's per-page consensus is what makes it solvable.
...
// deleteConsumerOn removes one consumer pod and waits for it to be gone,
// leaving any other consumers alone. The Shared step needs this because it
// deliberately runs two consumers at once and has to put the cluster back to
// exactly one before the migration step looks at it.
func (h *harness) deleteConsumerOn(t *testing.T, ctx context.Context, node string) {
	t.Helper()

	name := consumerName(node)

	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: claimNS}}
	if err := h.cli.Delete(ctx, pod); err != nil && !apierrors.IsNotFound(err) {
		t.Fatalf("delete consumer on %s: %v", node, err)
	}

	h.waitFor(t, ctx, 5*time.Minute, "consumer "+name+" to go away", func() error {
		got := &corev1.Pod{}

		err := h.cli.Get(ctx, types.NamespacedName{Name: name, Namespace: claimNS}, got)
		if apierrors.IsNotFound(err) {
			return nil
		}
		...
		return fmt.Errorf("%s is still terminating", name)
	})
}
