...
// placeNodes puts every worker in the suite's Site and pins which racer zone it
// lands in.
//
// The site label is what enrols a node: racer runs on every node in a Site that
// enables it, so labelling a worker with this suite's Site is the whole of the
// opt-in. The zone-name annotation is an operator input rather than a derived
...
func (h *harness) placeNodes() {
	...
	for i, name := range h.workers {
		patch := fmt.Sprintf(
			...
			component.SiteLabelKey, siteName,
