...
// TestValidateHostPrefix pins what may be configured as an installation prefix.
// The value is interpolated into generated systemd units and into a shell
// script, neither of which quotes it, so the accepted syntax is deliberately
// narrow enough to be inert in both rather than requiring two kinds of
// escaping that every consumer would have to keep correct.
func TestValidateHostPrefix(t *testing.T) {
	...
	for _, prefix := range []string{
		...
		"/opt/Unbounded_1.0-rc.2",
	} {
		if err := ValidateHostPrefix(prefix); err != nil {
			t.Errorf("ValidateHostPrefix(%q) = %v, want nil", prefix, err)
		}
	}
	...
	for _, tc := range []struct{ prefix, reason string }{
		{"usr/local", "relative"},
		{"./opt", "relative"},
		{"/opt/", "trailing separator is not normalized"},
		{"/opt/../opt", "unnormalized"},
		{"/", "filesystem root"},
		{"/opt/un bounded", "whitespace"},
		{"/opt/$HOME", "shell substitution"},
		{"/opt/%i", "systemd specifier"},
		{"/opt/un;rm -rf /", "shell metacharacter"},
		{"/opt/\"quoted\"", "quoting"},
		{"/opt/un`cmd`", "command substitution"},
	} {
		if err := ValidateHostPrefix(tc.prefix); err == nil {
			t.Errorf("ValidateHostPrefix(%q) = nil, want an error (%s)", tc.prefix, tc.reason)
		}
	}
}

// TestValidateRejectsBadHostPrefix checks the prefix is actually reached by
// whole-config validation, not merely validatable in isolation.
func TestValidateRejectsBadHostPrefix(t *testing.T) {
	t.Parallel()

	cfg := validAgentConfigForHostPrefix()
	if err := cfg.Validate(); err != nil {
		t.Fatalf("baseline config should be valid: %v", err)
	}

	cfg.HostPrefix = "/opt/$INJECTED"
	if err := cfg.Validate(); err == nil {
		t.Fatal("Validate() = nil, want an error for an unsafe HostPrefix")
	}

	cfg.HostPrefix = "/opt/unbounded"
	if err := cfg.Validate(); err != nil {
		t.Fatalf("Validate() = %v, want nil for a valid HostPrefix", err)
	}
}

func validAgentConfigForHostPrefix() *AgentConfig {
	return &AgentConfig{
		MachineName: "machine",
		NodeName:    "node",
		Cluster:     AgentClusterConfig{ClusterDNS: "10.96.0.10"},
		Kubelet:     AgentKubeletConfig{ApiServer: "https://api.example.test"},
	}
}
