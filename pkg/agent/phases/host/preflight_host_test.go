...
func TestCheckLocalDNSConntrackPrefersNativeNFTables(t *testing.T) {
	var commands []string

	deps := localDNSConntrackDeps{
		lookupPath: lookupPathWith(map[string]bool{"nft": true, "iptables": true}),
	...
	assert.Equal(t, preflight.SeverityOK, results[0].Severity)
	assert.Contains(t, results[0].Message, "native nftables")
	assert.Equal(t, []string{"nft"}, commands)
}

func TestCheckLocalDNSConntrackFallsBackToIPTables(t *testing.T) {
	deps := localDNSConntrackDeps{
		lookupPath: lookupPathWith(map[string]bool{"nft": true, "iptables": true}),
		run: func(_ context.Context, name string, _ []string, _ string) ([]byte, error) {
			if name == "nft" {
...
func TestCheckLocalDNSConntrackMissingBackends(t *testing.T) {
	...
	results = checkLocalDNSConntrackWithDeps(slog.New(slog.DiscardHandler), true, deps).Check(context.Background())
	assert.Equal(t, preflight.SeverityError, results[0].Severity)
}
