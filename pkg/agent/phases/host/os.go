...
func detectHostPackageManagerFor(
	...
	// Some hosts have no package manager at all. Immutable images mount /usr
	// read-only and may ship no tdnf, dnf, rpm or rpm-ostree, so there is
	// nothing to install with and nothing to install into. Such a host is still
	// usable when the capabilities the required packages exist to provide are
	// already present, whether baked into the image or supplied by a system
	// extension.
	//
