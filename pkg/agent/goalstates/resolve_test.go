...
// TestHostDistroIsImageManaged pins which hosts are treated as delivering their
// OS content as an image. On such a host the agent validates that required
// tools are present instead of trying to install them, so a host wrongly
// classified here would either attempt package installation into a read-only
// /usr or refuse to remediate a host that could have been remediated.
func TestHostDistroIsImageManaged(t *testing.T) {
	t.Parallel()

	assert.True(t, HostDistroIsImageManaged(hostDistroAzureContainerLinux))

	for _, distro := range []string{
		hostDistroAzureLinux3,
		hostDistroUbuntu2404,
		hostDistroUbuntu2604,
		"",
	} {
		assert.False(t, HostDistroIsImageManaged(distro), "distro %q", distro)
	}
}

// TestResolveNSpawnConfigResolvesTheLifecycleHelperFromThePrefix pins the one
// value in the nspawn goal state that is both installed as a file and named
// inside a generated unit.
//
// The hook units are written once and have to keep working across an agent
// upgrade, so the helper's location cannot be recomputed independently by the
// code that installs it and the code that references it. Resolving it here
// gives both a single answer.
func TestResolveNSpawnConfigResolvesTheLifecycleHelperFromThePrefix(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range map[string]struct {
		...
		"unset prefix keeps the historical path": {
			prefix: "",
			want:   "/usr/local/bin/unbounded-agent-nspawn-lifecycle",
		},
		"explicit default is indistinguishable from unset": {
			prefix: DefaultHostPrefix,
			want:   "/usr/local/bin/unbounded-agent-nspawn-lifecycle",
		},
		"configured prefix moves the helper": {
			prefix: "/opt/unbounded",
			want:   "/opt/unbounded/bin/unbounded-agent-nspawn-lifecycle",
		},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			got, err := ResolveNSpawnConfig(&config.AgentConfig{HostPrefix: tc.prefix}, NSpawnMachineKube1)
			require.NoError(t, err)
			require.Equal(t, tc.want, got.NSpawnLifecycleBinary)

			// The machine's own paths are inside the nspawn container and must
			// not move with the host prefix.
			require.Equal(t, "/var/lib/machines/kube1", got.MachineDir)
		})
	}
}
