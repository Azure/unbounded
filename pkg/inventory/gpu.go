...
func queryNvidiaSMI() ([]nvidiaSMIInfo, error) {
...
func queryLspci() ([]lspciGPU, error) {
	out, err := exec.Command("lspci", "-mm", "-D").Output()
	if err != nil {
		return nil, err
	}
...
func enrichFromSysfs(g *GPUInfo) {
	...
	if g.DriverVersion == "" && g.Driver != "" {
		if out, err := exec.Command("modinfo", "-F", "version", g.Driver).Output(); err == nil {
