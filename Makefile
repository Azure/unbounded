...
# Playpen (VXLAN-over-WireGuard overlay demo client; ships its own server-side
...
PLAYPEN_LDFLAGS=$(STAMP_LDFLAGS) -X main.DefaultPodImage=$(PLAYPEN_IMAGE)
...
.PHONY: playpen playpen-build image-playpen-local image-playpen-push
...
help: ## Show this help
	...
	@echo "  playpen | playpen-build          Build playpen (with/without lint/test)"
	...
	@echo "  image-playpen-push              Build and push the playpen image"
...
##@ Playpen (VXLAN-over-WireGuard overlay demo)

playpen-build: ## Build the playpen binary (no lint/test)
	$(GOBUILD) -ldflags '$(PLAYPEN_LDFLAGS)' -o $(PLAYPEN_BIN) $(PLAYPEN_CMD)
