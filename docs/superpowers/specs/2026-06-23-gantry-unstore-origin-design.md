...
**Two-slot primary/fallback struct** - simpler initially but requires a breaking API
change for a third backend. The slice-based chain handles one or many backends equally.

**Per-backend metric counter families** - e.g. `gantry_unstore_pull_total` per type.
A shared family with a `backend` label lets dashboards query across all cache origins
with one selector and requires no dashboard changes when a new backend is added.

**OCI Distribution Spec proxy in unbounded-storage** - pointing containerd's mirror
config at unbounded-storage directly. Larger scope, not required to achieve the goal.
