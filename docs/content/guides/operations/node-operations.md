...
Recreating the node rootfs is not a MachineOperation. Instead, delete the
Kubernetes Node object and let the agent reconcile.
...
| Need a new Kubernetes version | Delete the Node | Agent recreates with the desired MachineConfiguration. |
| Node rootfs is corrupted | Delete the Node | Agent replaces the rootfs entirely. |
