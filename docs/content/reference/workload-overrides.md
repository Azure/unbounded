...
| `sites` | no | **Per-Site components only**, meaning `metalman` and `racer-dataplane`. Naming it on `net`, `machina`, `gantry`, `token-refresher` or `racer-controlplane` is an error, because those are cluster singletons and there is one of each for the whole cluster. **Omit it to match every Site.** An empty list is an error, since it is far likelier to be a mistake than an intent to match nothing. |
...
`metalman` and `racer-dataplane` place their workloads with a mandatory per-Site node
