...
persisted config to resolve again. Bootstrap records its installation identity
and next stage under `/var/lib/unbounded/agent`. Repeating `start` with the same
configuration resumes an unfinished installation; the Ignition bootstrap unit
retries automatically. Late-stage retry preflight permits listeners only when
their process root and executable belong to that installation. Unknown or
unrelated listeners are still errors. Use explicit reset when abandoning an
installation, rather than as the normal response to a transient failure.
`NodeReboot` is intended for
...
After completed bootstrap, daemon repair uses the current applied configuration
and a separate `repairing-daemon` checkpoint. It does not rewrite the original
kube1 configuration after repave to kube2. Ambiguous applied slot configuration
is an error rather than a reason to select the first slot. CLI reset acquires
...
establish abrupt power-loss behavior; abrupt-loss acceptance is tracked separately.

The agent reports the current installation's provisioning format in
`Machine.status.observedProvisioningFormat`, including on pre-created Machines.
`spec.host.provisioningFormat` remains explicit desired replacement intent.
When preserving the image, replacement uses the observed format if no explicit
format is set. A known Ignition installation with an explicit replacement image
requires an explicit target format. Legacy Machines with neither observation nor
declaration retain the cloud-init default. Controller-generated Ignition
replacement is rejected before provider execution.
