...
kubectl unbounded net controller status-json # cluster summary JSON export
...
`controller status-json` preserves the controller's `nodeSummaries`, `nodeCount`,
build information, and other metadata, including fields unknown to the plugin.
Use `--pretty=false` for compact JSON. Older controllers' full `nodes` responses
are projected into summaries; peers, routes, and BPF arrays are not exported.
The command only fetches the cluster overview and does not request named-node
diagnostics. Use `node show <name> json` for those details.
