...
pub(crate) type Groups<'a> = dyn Fn(u64, crate::config::Kind) -> GroupId + 'a;
...
impl Snaps {
    ...
    pub(crate) fn next(
        ...
        let keep = |addr: u64| gof(addr, kind) == group && filter.keeps(addr);
...
fn klass(immutable: bool) -> Klass {
    Klass::of(if immutable {
        crate::config::Kind::Immutable
    } else {
        crate::config::Kind::Mutable
    })
}
...
fn repairs_per_replay(cfg: &Config) -> usize {
    cfg.repairs_per_replay() as usize
}
...
/// had got to. Lives in that worker's [`crate::server::CoreState`], so no cell here is ever
...
pub fn open(paxos: &'static Paxos) -> &'static Heal {
    Box::leak(Box::new(Heal { paxos }))
}

/// This worker's row, for the length of one synchronous step.
fn here<T>(worker: &Worker, f: impl FnOnce(&Core) -> T) -> T {
    f(&worker.core().heal)
}
...
impl Heal {
    ...
    pub fn outstanding(&'static self, worker: &Worker) -> (u64, u64) {
        let replaying = self.paxos.replaying_here(worker).len() as u64;
        let shedding = [false, true]
            .into_iter()
            .filter(|&huge| self.serves(huge))
            .flat_map(|huge| self.alloc().held_groups(worker, huge))
            .filter(|&g| {
                self.paxos
                    .members(worker, g)
                    .is_none_or(|m| self.paxos.self_index(worker, &m).is_none())
            })
            .count() as u64;
        (replaying, shedding)
    }
    ...
    fn stat(&self, worker: &Worker, f: impl FnOnce(&mut Stats)) {
        here(worker, |c| f(&mut c.stats.borrow_mut()));
    }

    /// Names the step the sweep is about to await, for the stall log.
    fn phase(&self, worker: &Worker, at: &'static str) {
        here(worker, |c| c.phase.set(at));
    }
    ...
    pub fn tick(&'static self, worker: Rc<Worker>, now: Instant) {
        ...
        if !runtime::spawn_local(async move {
            if runtime::deadline(self.sweep(task_worker.clone()), SWEEP_DEADLINE)
                .await
                .is_none()
            {
                ...
                let phase = here(&task_worker, |c| c.phase.get());
                self.stat(&task_worker, |s| s.stalled += 1);
                eprintln!("racer: heal sweep abandoned at {phase}");
            }
            here(&task_worker, |c| c.busy.set(false));
        }) {
            here(&worker, |c| c.busy.set(false));
        }
    }
    ...
    fn next_group(
        ...
        loop {
            if let Some(cand) = group_at(cfg, n)
                ...
                && self.paxos.members(worker, cand).is_some()
            {
    ...
    async fn sweep(&'static self, worker: Rc<Worker>) -> Result<(), Status> {
        ...
        for huge in [false, true] {
            if self.serves(huge) {
                self.phase(&worker, if huge { "shed(huge)" } else { "shed(small)" });
                self.shed(worker.clone(), huge).await;
            }
        }
        if self.alloc().pressure(&worker) != Pressure::Normal {
            return Ok(());
        }
        ...
        let g = match self
            .paxos
            .replaying_here(&worker)
            .into_iter()
            .find(|&g| self.paxos.members(&worker, g).is_some())
        {
            Some(r) => r,
            ...
            None => {
                let picked =
                    self.next_group(&worker, cfg, runtime::core(), &worker.core().heal, groups);
                match picked {
                    Some(g) => g,
                    None => return Ok(()),
                }
        ...
        self.stat(&worker, |s| s.sweeps += 1);
        self.phase(&worker, "replaying");
        ...
        for huge in [false, true] {
            ...
            match self.compare(worker.clone(), cfg, g, huge, was).await {
                Ok(r) => replaying |= r,
                Err(_) => {
                    checked = false;
                    self.stat(&worker, |s| s.failed += 1);
                }
            }
        }
        // A class we could not compare is not evidence of having caught up on it.
        let still = replaying || (was && !checked);
        self.phase(&worker, "replay-flag");
        if still {
            self.paxos.set_replaying(g, true).await;
        } else if was && self.paxos.rejoin(worker.clone(), g).await.is_err() {
            // Leaving a replay needs the promise recovered first; the next sweep retries.
            self.stat(&worker, |s| s.failed += 1);
        }
        self.phase(&worker, "hand-over");
        self.hand_over(worker.clone(), cfg, g).await;
        self.phase(&worker, "done");
        Ok(())
    }
    ...
    async fn hand_over(&'static self, worker: Rc<Worker>, cfg: &Config, group: GroupId) {
        ...
        for ext in &u.extents {
            ...
            if !self.paxos.sealed(ext.id).await {
                ...
                let _ = self.paxos.seal_extent(worker.clone(), lo, ext.id).await;
                continue;
            }
            ...
            let _ = self
                .push_extent(
                    worker.clone(),
                    group,
                    ext.guard() == crate::config::Kind::Immutable,
                    filter,
                    ext.next_zone,
                )
                .await;
    ...
    async fn push_extent(
        ...
        let snap = self
            .alloc()
            .snap_open(worker.clone(), group, huge, filter)
            .await?;
        let mut sent = 0usize;
        loop {
            ...
            for (addr, r) in tuples {
                let _ = self
                    .paxos
                    .push(worker.clone(), GlobalAddr(addr), r, zone)
                    .await;
                sent += 1;
            }
            if done || sent >= repairs_per_replay(worker.config()) {
                break;
            }
        }
        snap.close().await;
    ...
    fn serves(&self, huge: bool) -> bool {
        let c = if huge { Class::Immutable } else { Class::Mutable };
    ...
    async fn shed(&'static self, worker: Rc<Worker>, huge: bool) {
        let orphans: Vec<GroupId> = self
            .alloc()
            .held_groups(&worker, huge)
            .into_iter()
            .filter(|&g| {
                self.paxos
                    .members(&worker, g)
        ...
        for g in orphans {
            ...
            if self
                .drain(worker.clone(), g, huge, &mut budget)
                .await
                .is_err()
            {
                self.stat(&worker, |s| s.failed += 1);
    ...
    async fn drain(
        ...
        let snap = self
            .alloc()
            .snap_open(worker.clone(), group, huge, Filter::All)
            .await?;
        ...
        while *budget > 0 {
            ...
            for (addr, r) in tuples {
                ...
                *budget -= 1;
                ...
                if self.paxos.confirmed(worker.clone(), a, r.version).await
                    && self.alloc().discard(&worker, a, r.version).await.is_ok()
                {
                    self.stat(&worker, |s| s.dropped += 1);
                }
            }
            if done {
                walked = true;
        ...
        if walked {
            self.alloc().forget_group(&worker, group, huge).await;
        }
        Ok(())
    }
    ...
    async fn compare(
        ...
        let m = self.paxos.members(&worker, group).ok_or(Status::Unmapped)?;
        let me = self.paxos.self_index(&worker, &m).ok_or(Status::Unmapped)?;
        // The partner alternates with group parity, so the group walks all three edges.
        let from = ((me as u32 + 1 + group.index() % 2) % 3) as u8;
        let link = self
            .paxos
            .link_of(&worker, group.universe(), m[from as usize])
            .ok_or(Status::Io)?;

        let mine = self.alloc().digests(&worker, group, huge).await;
        ...
        self.stat(&worker, |s| s.buckets_diff += diff.len() as u64);
        ...
        for b in diff {
            ...
            self.reconcile(worker.clone(), cfg, group, huge, link, b, &mut budget)
                .await?;
        }
        Ok(replay)
    }
    ...
    async fn reconcile(
        ...
        let Some(theirs) = self.remote_bucket(link, group, huge, bucket).await? else {
            self.stat(&worker, |s| s.oversized += 1);
            return Ok(());
        };
        let Some(mine) = self
            .local_bucket(worker.clone(), group, huge, bucket)
            .await?
        else {
            self.stat(&worker, |s| s.oversized += 1);
            return Ok(());
        ...
        for (addr, r) in theirs.iter() {
            ...
            if mine.get(addr) != Some(r) {
                *budget -= 1;
                self.repair(worker.clone(), cfg, GlobalAddr(*addr)).await;
            }
        }
    ...
    async fn repair(&'static self, worker: Rc<Worker>, cfg: &Config, addr: GlobalAddr) {
        // A page whose extent left the config no longer exists; not a divergence.
        if cfg.extent_at(addr.0).is_none() {
            return;
        }
        match self.paxos.repair(worker.clone(), addr).await {
            Ok(_) => self.stat(&worker, |s| s.repairs += 1),
            Err(_) => self.stat(&worker, |s| s.failed += 1),
        }
    }
    ...
    async fn local_bucket(
        ...
        let snap = self
            .alloc()
            .snap_open(worker, group, huge, Filter::Bucket(bucket))
            .await?;
        let mut out = BTreeMap::new();
        let mut over = false;
        loop {
