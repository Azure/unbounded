...
impl<T: DiskTarget> DiskRegistry<T> {
    ...
    /// bypass_index_read / bypass_checksum) are treated as a remove
    /// followed by an add. `disable_page_cache` is live routing metadata
    /// and updates the channel snapshot without reopening the disk.
