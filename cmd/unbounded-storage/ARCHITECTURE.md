...
- `[[caches]]` - `name`, `source` (a backend or neighborhood component name),
  and `disk_pool` naming the `[[disk_pools]]` entry whose disks back the cache.
- `[[disk_pools]]` - `name` and `[[disk_pools.disks]]`. Each disk has one
  ...
  Disk paths must be unique across all pools.
