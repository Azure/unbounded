...
pub(crate) mod dst {
    ...
    impl Cluster {
        ...
        pub(crate) fn handler(&self, n: usize) -> std::cell::RefMut<'_, Handler> {
            self.machines[n].driver.application().volumes.servers
                [&Address::unix(&self.machines[n].config.volumes[0].cache_socket).unwrap()]
                .handler()
                .current
                .handlers[0]
                .borrow_mut()
        ...
        fn generation(&self, n: usize) -> Option<Rc<Generation>> {
            ...
            Some(
                m.driver
                    ...
                    .volumes
                    .servers
                    .get(&Address::unix(&m.config.volumes[0].cache_socket).unwrap())?
                    .handler()
                    .current
                    .clone(),
            )
...
fn automatic_cross_worker_handshake_confirmation_reconnect_and_drain() {
    ...
    let b_rails =
        negotiation::Rails::new(vec![None, None, Some(rdma::test_transport(ring.pool()))], 3)
            .unwrap();
    let mut a = activate(&mut ring, prepared(2, aa, ba, true), aa, 5, a_rails);
    let mut b = activate(&mut ring, prepared(3, ba, aa, false), ba, 9, b_rails);
...
fn stale_inbound_replacement_preserves_reverse_session_at_two_qp_capacity() {
    ...
    let mut a = activate(&mut ring, prepared(2, aa, ba, true), aa, 0, ar);
    let mut b = activate(&mut ring, prepared(3, ba, aa, true), ba, 0, br);
    warm(&a, aa);
    warm(&b, ba);
    let ag = generation(&a, aa);
    let bg = generation(&b, ba);
...
fn old_tcp_finish_routes_to_pinned_server_without_stale_install() {
    ...
    let mut b = activate(&mut ring, prepared(3, ba, aa, false), ba, 7, rails);
