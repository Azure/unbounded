...
//!
//! RACERS01 stores Content-Type object records and 64 MiB payload extents.
...
const MAGIC: u64 = u64::from_le_bytes(*b"RACERS01");
const NODE: u64 = u64::from_le_bytes(*b"RACERN01");
...
fn reject_version(page: &uring::Page) -> io::Result<()> {
    if page.0[..6] == *b"RACERS" && get(page, 0) != MAGIC {
        return Err(invalid(
            "incompatible slab format: expected RACERS01 with Content-Type metadata and 64 MiB payloads; preserve the old slab and use a new RACER_SLAB_PATH; no automatic migration/reformat",
        ));
    }
    Ok(())
}
    ...
    fn new(size: u64, count: usize, shard: usize) -> io::Result<Self> {
        ...
        if bitmap_pages > ROOT_BITMAP_SLOTS as u64 {
            return Err(io::Error::new(
                ...
                format!(
                    "slab geometry: {size} bytes across {count} shards gives {len} bytes/shard requiring {bitmap_pages} bitmap pages; RACERS01 roots hold at most {ROOT_BITMAP_SLOTS}, limiting each shard to {MAX_SHARD_SIZE} bytes (64 MiB aligned). Reduce slab size or increase RACER_SHARDS for a new slab; preserve an existing slab and use a new RACER_SLAB_PATH rather than changing its layout"
                ),
            ));
        }
        Ok(Self {
