...
impl Handler {
    ...
    fn route_state(
        ...
        let mut cursor = match cursor {
            Some(cursor) if peer => routing.receive(cursor, target)?,
    ...
    pub fn poll_background(&mut self, ring: &mut Ring, budget: usize) -> io::Result<Work> {
        ...
        if !self.draining && !self.connections.is_empty() {
            ...
            if let Ok(Some(request)) = connection.next_request() {
                ...
                if let Ok((_, descriptor)) = routed_descriptor(&request.metadata)
                    && let Ok(upstream) = self.peer_provider(&request.metadata)
                    && let Ok(deadline) =
        ...
        if self.pages.len() < WINDOW && self.next < self.end {
            ...
            if leading_owns_buffer {
                ...
                let fault = cache.page(
                    ...
                    self.next,
                    self.response_deadline.get().min(self.deadline),
                )?;
