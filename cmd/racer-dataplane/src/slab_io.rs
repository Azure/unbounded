...
/// Hold an exhausted test bucket across real-kernel scheduling and cancellation.
/// Dropping resumes real-time refill without credit for time spent paused.
#[cfg(test)]
pub(crate) struct PausedRefill(Arc<Bucket>);
#[cfg(test)]
impl Drop for PausedRefill {
    fn drop(&mut self) {
        let mut state = self.0.state.lock().unwrap();
        state.updated = crate::environment::now();
        state.refill_paused = false;
    }
}
...
impl Io {
    #[cfg(test)]
    pub(crate) fn exhaust_and_pause_refill(&self) -> PausedRefill {
        let bucket = self.bucket.as_ref().expect("limited test bucket");
        let mut state = bucket.state.lock().unwrap();
        assert!(!state.refill_paused, "refill is already paused");
        state.tokens = 0;
        state.refill_paused = true;
        PausedRefill(bucket.clone())
    }
    #[cfg(test)]
