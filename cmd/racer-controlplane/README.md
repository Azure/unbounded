...
configuration. Enrollment uses Pod-token checks; subscriptions use the enrolled
certificate. With no selected caches, managed Pods receive `idle`
...
All caches share the dedicated peer mTLS listener on port 9443. Local ingress
uses each cache's filesystem Unix socket; there are no per-cache TCP listeners
or port allocations. Management normally uses port 9090. Persisted topology
format 4 requires fresh controller state when replacing an older format.
...
fallback. UID-only members retain
...
commit. Revisions, slot ownership, cache socket paths, and rollout decisions
...
placement continuity. This API requires format-4 durable generations. Earlier
