...
func runMultiForward(t *testing.T, bin string, trap uint32) {
	...
	kube := multiTokenClient{coordinationKube(n, n2, p, p2, bad, good)}
	...
	mux.HandleFunc("GET /multi/failed", func(w http.ResponseWriter, r *http.Request) {
		...
		t.Log("multi: failed staging observed; corrective cache deletion committed")
	...
	mux.HandleFunc("GET /v3/{universe}/{node}", func(w http.ResponseWriter, req *http.Request) {
		...
		started := time.Now()
		// Reconciliation is controller work, independent of a subscriber closing
		// its bounded request while a topology commit is in progress.
		if _, err := rec.Reconcile(ctx, request); err != nil {
			t.Error(err)
			w.WriteHeader(503)
		...
		if rr.Code == 200 {
			...
			cm := &corev1.ConfigMap{}
			if err := kube.Get(ctx, client.ObjectKey{Namespace: "state", Name: stateName("default") + "-rollout"}, cm); err != nil {
				t.Error(err)
			}
		...
		_, writeErr := w.Write(rr.Body.Bytes())
		if time.Since(started) > time.Second || writeErr != nil {
			t.Logf("multi %s input=%d/%d corrected=%v reconcile=%s control=%s assertions/write=%s bytes=%d write=%v", role, rev, ack, corrected, reconciled.Sub(started), controlled.Sub(reconciled), time.Since(controlled), rr.Body.Len(), writeErr)
		}
	})
