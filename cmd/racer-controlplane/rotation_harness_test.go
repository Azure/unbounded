...
// Fixture snapshots delivered over mTLS give each process a distinct ingress
// port on this host. The coordination harness covers durable rollout decisions.
func TestProductionCARotationTraffic(t *testing.T) {
	...
	origin := httptest.NewServer(originfixture.NewOrigin())
	defer origin.Close()
	...
	dirs, addresses, metrics := make([]string, 2), make([]string, 2), make([]string, 2)
	clients := make([]*sdk.Client, 2)

	for i := range 2 {
		dirs[i] = configs[ids[i]]
		addresses[i] = rotationAddress(t, "127.0.0.1")
	...
	for i := range 2 {
		...
		snapshot := &pb.Snapshot{
			...
			Volumes: []*pb.Volume{{
				Id: "system/volume", Listen: addresses[i], OriginAddress: origin.Listener.Addr().String(), OriginIdentity: "system/origin:80", CacheGeneration: 1,
		...
		cmd.Env = append(cmd.Env,
			...
			"RACER_SLAB_PATH="+filepath.Join(dirs[i], "cache.slab"), "RACER_SLAB_SIZE=134217728", "RACER_SHARDS=1", "RACER_IO_WORKERS=1", "RACER_COMPUTE_WORKERS=1", "RACER_BUFFERS_PER_NODE=8", "RACER_RDMA_MODE=disabled", "RACER_METRICS_ADDR="+metrics[i])
		...
		clients[i], err = sdk.NewClient("http://"+addresses[i], sdk.ClientOptions{})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Logf("%d continuous SDK reads survived CA generations %d through %d, old-root retirement, and live leaf renewal", reads.Load(), initial.Generation, current.Generation)
}
