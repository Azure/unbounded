...
// runCleanup removes the shared resources playtime deliberately never cleans up
...
// lose its shared RBAC, cleanup first deletes every playtime run in the scope
...
func runCleanup(ctx context.Context, cfg Config) error {
	...
	if len(deletedShared) == 0 {
		fmt.Printf("no shared playtime resources found in namespace scope %q\n", cfg.Namespace)
		return nil
	}

	fmt.Printf("deleted %d shared playtime resource(s):\n", len(deletedShared))
