...
| Kubernetes RBAC scope | `serviceaccount.yaml` | The agent only consumes `pods.list/watch` (informer) plus `pods.patch` (self-announce of libp2p, transfer, and zone metadata) in its own namespace. Zone-scoped HRW additionally uses `nodes.get` once for the agent's own Node; there is no cluster-wide Node informer. There is no `get` on pods - informer events deliver the objects without point reads. Review `ClusterRole/Role` to confirm scope hasn't drifted. Membership setup failure is fatal in production mode (Downward-API env vars set), so an RBAC misconfig surfaces as a CrashLoop on rollout instead of a silent single-node fallback. |
