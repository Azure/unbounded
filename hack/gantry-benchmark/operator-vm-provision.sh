...
OPERATOR_VM_SIZE="${OPERATOR_VM_SIZE:-Standard_D8ds_v5}"
OPERATOR_OS_DISK_GB="${OPERATOR_OS_DISK_GB:-512}"
...
if [[ "$START_BENCHMARK" == true ]]; then
  az vm run-command invoke \
    ...
    -n "$OPERATOR_VM_NAME" \
    --command-id RunShellScript \
    --scripts 'systemctl reset-failed gantry-benchmark-operator.service || true' 'systemctl start --no-block gantry-benchmark-operator.service' \
    --only-show-errors \
    -o none
fi
