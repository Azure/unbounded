...
#
# add-azure-site-kubeadm.sh - Deploy a complete Azure external site for a kubeadm-based primary cluster
#
...
# It resolves the kubelet identity, NSG, and bootstrap configuration from the
# primary site (deployed by create-primary-site-azure-kubeadm.sh), generates
# per-pool customdata inline, and deploys everything in a single Bicep deployment.
#
# Usage:
#   ./add-azure-site-kubeadm.sh --name <site-name> --ipv4-range <cidr> --primary-site <name> [options]
#
# Required arguments:
...
#   --primary-site          Name of the primary site (deployed by create-primary-site-azure-kubeadm.sh)
#
# Optional arguments:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --primary-site)      PRIMARY_SITE="$2"; shift 2 ;;
...
[[ -n "$PRIMARY_SITE" ]] || die "--primary-site is required"
...
# Resolve site subscription and tenant from az CLI context
SITE_SUBSCRIPTION_ID="$(az account show --query id -o tsv)"
TENANT_ID="$(az account show --query tenantId -o tsv)"

# Resolve the kubelet managed identity from the primary site
# (created by create-primary-site-azure-kubeadm.sh as {primary-site}-identity)
PRIMARY_IDENTITY_NAME="${PRIMARY_SITE}-identity"

# The primary site's resource group may differ from the external site's RG.
# Try to find the identity in the current RG first, then fall back to searching.
IDENTITY_ID="$(az identity show "${RG_FLAG[@]}" -n "$PRIMARY_IDENTITY_NAME" --query id -o tsv 2>/dev/null)" || true
if [[ -z "$IDENTITY_ID" ]]; then
    # Search across the subscription
    IDENTITY_ID="$(az identity list --query "[?name=='${PRIMARY_IDENTITY_NAME}'].id" -o tsv 2>/dev/null | head -1)"
fi
[[ -n "$IDENTITY_ID" ]] || die "Could not resolve identity ${PRIMARY_IDENTITY_NAME}"

KUBELET_CLIENT_ID="$(az identity show --ids "$IDENTITY_ID" --query clientId -o tsv)"
[[ -n "$KUBELET_CLIENT_ID" ]] || die "Could not resolve client ID for identity $IDENTITY_ID"

IDENTITY_PRINCIPAL_ID="$(az identity show --ids "$IDENTITY_ID" --query principalId -o tsv)"
[[ -n "$IDENTITY_PRINCIPAL_ID" ]] || die "Could not resolve principal ID for identity $IDENTITY_ID"

# Resolve primary site VNet for peering
CLUSTER_VNET_NAME="${PRIMARY_SITE}-vnet"
CLUSTER_VNET_RG="$(az network vnet show -n "$CLUSTER_VNET_NAME" --query 'resourceGroup' -o tsv 2>/dev/null \
    || az network vnet list --query "[?name=='${CLUSTER_VNET_NAME}'].resourceGroup" -o tsv 2>/dev/null | head -1)"
[[ -n "$CLUSTER_VNET_RG" ]] || die "Could not resolve resource group for VNet $CLUSTER_VNET_NAME"
PRIMARY_SITE_NAME="$PRIMARY_SITE"

# Cross-subscription not applicable for kubeadm clusters (identity in same sub)
PRIMARY_SUB_FLAG=()
CROSS_SUBSCRIPTION=false
PRIMARY_SUBSCRIPTION_ID="$SITE_SUBSCRIPTION_ID"
...
# Use the resource group as CLUSTER_RG for kubeadm clusters (needed by customdata template)
CLUSTER_RG="$RESOURCE_GROUP"
...
echo "    IPv6 range:              ${IPV6_RANGE:-(unset)}"
