...
#
# Usage:
#   ./hack/agent/qemu/vm.sh <command> [options]
#
# Commands:
...
#
# Start options:
#   -n, --name        VM name (default: agent-vm)
...
#   -u, --user-data   Path to cloud-init user-data file (default: hack/agent/qemu/user-data.yaml)
...
Stop options:
  -n, --name   VM name (default: agent-vm)
  -f, --force  Force kill (SIGKILL) instead of graceful shutdown (SIGTERM)
      --clean  Also remove disk, seed ISO, log, and rendered cloud-init files
...
EOF
    ...
    QEMU_CMD=("${QEMU_BIN}"
        ...
        ${NET_ARGS}
        -virtfs "local,path=${REPO_ROOT},mount_tag=agent,security_model=mapped-xattr,id=agent0"
        -daemonize
    ...
    if [[ ${ATTEMPT} -ge ${MAX_ATTEMPTS} ]]; then
        ...
        echo ""
        echo "To stop the VM:"
        echo "  ./hack/agent/qemu/vm.sh stop -n ${VM_NAME}"
        exit 1
    fi
