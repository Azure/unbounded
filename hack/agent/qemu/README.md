...
A self-contained toolkit for creating, managing, and tearing down local QEMU virtual machines running Ubuntu 24.04 (Noble). Useful as a local development and testing environment for the unbounded agent.
...
./hack/agent/qemu/vm.sh logs

# Stop the VM
./hack/agent/qemu/vm.sh stop

# Stop and clean up all artifacts
./hack/agent/qemu/vm.sh stop --clean
```
...
|---|---|---|---|
| `-n` | `--name` | `agent-vm` | VM instance name |
...
| `-u` | `--user-data` | `hack/agent/qemu/user-data.yaml` | Path to cloud-init user-data template |
...
| Flag | Long Form | Default | Description |
|---|---|---|---|
| `-n` | `--name` | `agent-vm` | VM name to stop |
...
```bash
./hack/agent/qemu/vm.sh start --name my-vm --memory 4096 --cpus 4 --disk-size 40G --ssh-port 2223
```

**Run multiple VMs simultaneously** (use different names and SSH ports):

```bash
./hack/agent/qemu/vm.sh start -n vm1 -p 2222
./hack/agent/qemu/vm.sh start -n vm2 -p 2223

# Stop individually
./hack/agent/qemu/vm.sh stop -n vm1
./hack/agent/qemu/vm.sh stop -n vm2 --clean
```

**Follow serial console output:**

```bash
./hack/agent/qemu/vm.sh logs -n my-vm --follow
```

## Development Workflow

The repository root is shared into the guest at `/agent` via virtio-9p. This means you can compile the binary on your host machine and immediately run it inside the VM -- no need to copy files or install a Go toolchain in the guest.

```bash
# On the host: build the binary (targeting Linux)
GOOS=linux go build -o bin/unbounded-agent ./cmd/agent
...
cd /agent
sudo su
./bin/unbounded-agent --config <path-to-config>
```

Edit code and rebuild on the host as usual. Since `/agent` is a live mount of the repository, the updated binary is available inside the VM immediately after each build.
...
5. **QEMU launch** -- Starts QEMU daemonized with virtio disk, seed ISO, SSH port forwarding, hardware acceleration, and a **9p shared filesystem** that mounts the repository root into the guest at `/agent`.
...
- **Shared filesystem:** The repository root is mounted at `/agent` inside the guest via virtio-9p
- **Provisioning marker:** `/etc/agent/provisioned` is written on first boot
