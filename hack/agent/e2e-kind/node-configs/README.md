...
| `sandboxImage` | string | Optional containerd CRI sandbox image reference. The e2e mirrors it into a local registry and passes the local reference to `manual-bootstrap --sandbox-image`. |
...
`--offline-artifacts-oci-ref`, `--offline-rootfs-oci-image`, and the node config
