...
| `kubeletConfiguration` | object | Optional `Kubelet.Configuration` overlay injected into the generated agent config. The e2e validates persistence, generated kubelet YAML, kubelet `ExecStart`, and `maxPods` through the Node pod capacity. |
| `offlineArtifactsOCIRef` | string | Optional OCI artifact bundle reference for offline bootstrap binaries. The e2e mirrors it into a local registry and passes the local `oci://` reference to `manual-bootstrap --offline-artifacts-source`, including the Ignition variant. For blocked-network scenarios, the e2e builds local bundles during the test for both the bootstrap Kubernetes version and the repave target version. Those bundles include cluster-only kube-system images plus the e2e workload image, then the e2e passes a templated local bundle reference. |
