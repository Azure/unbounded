...
func addInventoryCmd(siteCmdContext *siteCommandContext, site *azuredev.Datacenter) *cobra.Command {
	...
	c.Flags().StringVar(&machinaSSHSecretRef, "machina-ssh-secret-ref", "", "Secret reference for spec.ssh.privateKeyRef in format [$namespace/]$name[:$key] (default namespace: unbounded-system)")
	c.Flags().StringVar(&machBastionSSHSecret, "machina-bastion-ssh-secret-ref", "", "Secret reference for spec.ssh.bastion.privateKeyRef in format [$namespace/]$name[:$key] (default namespace: unbounded-system)")
