...
| `--machina-ssh-secret-ref` | | | Secret reference for `spec.ssh.privateKeyRef` in format `[$namespace/]$name[:$key]` (default namespace: unbounded-system) |
| `--machina-bastion-ssh-secret-ref` | | | Secret reference for `spec.ssh.bastion.privateKeyRef` in format `[$namespace/]$name[:$key]` (default namespace: unbounded-system) |
