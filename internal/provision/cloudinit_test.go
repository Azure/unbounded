...
func TestReplaceCloudInit(t *testing.T) {
	t.Parallel()

	cloudInit, err := ReplaceCloudInit(UnboundedAgentConfig{
	...
	require.Contains(t, cloudInit, replaceAgentConfigPath)
	require.Contains(t, cloudInit, replaceInstallPath)
