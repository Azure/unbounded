...
// a component's legacy resources may be reaped are Ready. Net is gated only on
// the new net workloads being created (not Ready)
...
// ConfigMaps with dynamic names (machine cloud-init), so a per-name
// "not migrated" classification would produce
