...
// testSite is the site the test nodes belong to. testEnrollment enables racer
// for it and nothing else, which is what one Site with racer switched on
// produces.
const testSite = "site-a"

func testEnrollment() enrollment {
	return enrollment{sites: map[string]bool{testSite: true}}
}

// enrolledNode builds a ready node racer belongs on: it sits in the site that
// enables racer, carries the zone label placement balances cohorts against, and
// its agent has announced itself. Whatever identity annotations the test wants
// are merged in on top of the announcement.
func enrolledNode(name, zone string, annotations map[string]string) *corev1.Node {
	merged := map[string]string{racerctrl.NodeAgentAnnotation: racerctrl.NodeAgentRunning}
	...
	return &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:        name,
			Labels:      map[string]string{component.SiteLabelKey: testSite, ZoneLabel: zone},
			Annotations: merged,
		},
...
// racerSite is a Site that switches racer on or off.
func racerSite(name string, enabled bool) unboundedv1alpha3.Site {
...
// The Sites are what say where racer belongs. A cluster where one site enables
// racer and another does not must end up with racer on the first site's nodes
// and nowhere near the second's, with nobody labelling a node by hand.
func TestReconcileEnrolsOnlyNodesInRacerSites(t *testing.T) {
	ctx := context.Background()

	outside := enrolledNode("n2", "east", nil)
	outside.Labels[component.SiteLabelKey] = "site-b"

	env := testEnv(t, enrolledNode("n1", "east", nil), outside)

	result := (Component{}).Reconcile(ctx, env, []unboundedv1alpha3.Site{
		racerSite(testSite, true),
		racerSite("site-b", false),
	})
	if result.Err != nil {
		t.Fatalf("reconcile: %v", result.Err)
	}

	if labels := nodeLabels(ctx, t, env, "n1"); labels[WorkloadLabel] != "true" {
		t.Fatalf("a node in the racer site is not running racer: %v", labels)
	}

	if labels := nodeLabels(ctx, t, env, "n2"); labels[WorkloadLabel] != "" {
		t.Fatalf("a node in a site that does not enable racer is running racer: %v", labels)
	}

	if _, ok := nodeAnnotations(ctx, t, env, "n2")[racerctrl.NodeIDAnnotation]; ok {
		t.Fatalf("a node in a site that does not enable racer was given an identity")
	}
}
...
func TestEnsureDefaultClassCreatesOneUniverse(t *testing.T) {
	...
	second, err := loadState(ctx, env, testEnrollment())
	if err != nil {
...
func TestPlaceVolumeIsIdempotent(t *testing.T) {
	...
	first, err := loadState(ctx, env, testEnrollment())
	if err != nil {
		t.Fatalf("load state: %v", err)
	}
...
// decommissioningNode is a node that has been opted out but still holds the
...
func decommissioningNode(name string, id uint32) *corev1.Node {
	...
	node.Labels[EnrollmentLabel] = "false"
...
// Opting a node out is a request to decommission, not an instruction to stop.
// The node has to keep running racer to accept the configs that step it out of
// each catalog and to shed what it holds, so the label the DaemonSet selects on
// has to outlive eligibility.
...
// A node racer does not belong on and that holds no identity has no business
// running racer, however its label got there.
func TestWorkloadLabelIsRemovedFromStrayNodes(t *testing.T) {
	ctx := context.Background()

	stray := enrolledNode("n1", "east", nil)
	delete(stray.Labels, component.SiteLabelKey)
	...
	if _, ok := nodeLabels(ctx, t, env, "n1")[WorkloadLabel]; ok {
		t.Fatalf("a node outside every racer site and with no identity is still labelled for the workload")
	}
}

// admits is the whole of the enrollment decision, so every case is worth
// stating in one place.
func TestEnrollmentAdmits(t *testing.T) {
	racerSite := enrollment{sites: map[string]bool{testSite: true}}
	frozen := enrollment{sites: map[string]bool{}, frozen: true}
	...
	cases := []struct {
		...
		{"a node in a racer site", racerSite, map[string]string{component.SiteLabelKey: testSite}, true},
		{"the deprecated site label still counts", racerSite, map[string]string{component.DeprecatedSiteLabelKey: testSite}, true},
		{"a node in another site", racerSite, map[string]string{component.SiteLabelKey: "site-b"}, false},
		{"a node in no site at all", racerSite, nil, false},
		{"opting out of a racer site", racerSite, map[string]string{component.SiteLabelKey: testSite, EnrollmentLabel: "false"}, false},
		{"the old opt-in label is inert", racerSite, map[string]string{EnrollmentLabel: "true"}, false},
		{"a frozen pass admits everything", frozen, nil, true},
		{"a frozen pass still honours the opt-out", frozen, map[string]string{EnrollmentLabel: "false"}, false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			node := &corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: "n1", Labels: tc.labels}}
			if got := tc.enroll.admits(node); got != tc.want {
				t.Fatalf("admits(%v) = %v, want %v", tc.labels, got, tc.want)
			}
		})
	}
}

// Running racer is the enrollment. A node in a site that enables racer carries
// no racer label of its own and still gets the workload and an identity.
func TestSiteMembershipEnrolsANodeWithoutALabel(t *testing.T) {
	ctx := context.Background()

	node := enrolledNode("n1", "east", nil)
	if _, ok := node.Labels[EnrollmentLabel]; ok {
		t.Fatalf("the test node carries %s, which is no longer how a node is enrolled", EnrollmentLabel)
	}
	...
	if labels := nodeLabels(ctx, t, env, "n1"); labels[WorkloadLabel] != "true" {
		t.Fatalf("a node in a racer site is not labelled for the workload: %v", labels)
	}

	if id := nodeAnnotations(ctx, t, env, "n1")[racerctrl.NodeIDAnnotation]; id == "" || id == "0" {
		t.Fatalf("a node in a racer site was given no identity")
	}
}

// An identity puts a node in a catalog, and a catalog entry is a promise that
// the node can serve. So the pod goes out first and the identity waits until
// the agent has announced that it is running, which it does only once preflight
// has claimed the store.
func TestIdentityWaitsForTheAgentToAnnounce(t *testing.T) {
	ctx := context.Background()

	silent := enrolledNode("n1", "east", nil)
	delete(silent.Annotations, racerctrl.NodeAgentAnnotation)

	env := testEnv(t, silent)
	...
	if labels := nodeLabels(ctx, t, env, "n1"); labels[WorkloadLabel] != "true" {
		t.Fatalf("a silent node was not given the pod that would let it speak: %v", labels)
	}

	if _, ok := nodeAnnotations(ctx, t, env, "n1")[racerctrl.NodeIDAnnotation]; ok {
		t.Fatalf("a node whose agent has never reported was put in a catalog")
	}

	// The agent comes up and says so, and the next pass places it.
	announce(ctx, t, env, "n1")
	...
	if err := second.allocateNodeIdentities(ctx); err != nil {
		t.Fatalf("allocate identities: %v", err)
	}

	if id := nodeAnnotations(ctx, t, env, "n1")[racerctrl.NodeIDAnnotation]; id == "" || id == "0" {
		t.Fatalf("an announced node was not placed")
	}
}

// A site that turns racer off is asking for its nodes back, which is the same
// request as the opt-out label and takes the same route: the node keeps the pod
// until it has given up its identity.
func TestDisablingASiteRetiresItsNodes(t *testing.T) {
	ctx := context.Background()

	node := decommissioningNode("n1", 7)
	delete(node.Labels, EnrollmentLabel)

	env := testEnv(t, node)

	p, err := loadState(ctx, env, enrollment{sites: map[string]bool{}})
	if err != nil {
		t.Fatalf("load state: %v", err)
	}
	...
	if _, ok := nodeAnnotations(ctx, t, env, "n1")[racerctrl.NodeIDAnnotation]; ok {
		t.Fatalf("a node whose site stopped enabling racer kept its id")
	}

	if _, ok := nodeLabels(ctx, t, env, "n1")[WorkloadLabel]; ok {
		t.Fatalf("a retired node is still selected by the DaemonSet")
	}
}

// The last Site turning racer off does not mean drain the cluster. There would
// be nowhere to drain to, and racer holds data, so a pass with no site behind it
// freezes: every node keeps its identity and its pod until someone removes the
// installation deliberately.
func TestFrozenEnrollmentLeavesEveryNodeInPlace(t *testing.T) {
	ctx := context.Background()

	node := decommissioningNode("n1", 7)
	delete(node.Labels, EnrollmentLabel)
	delete(node.Labels, component.SiteLabelKey)

	env := testEnv(t, node)

	p, err := loadState(ctx, env, enrollment{sites: map[string]bool{}, frozen: true})
	if err != nil {
		t.Fatalf("load state: %v", err)
	}
	...
	if id := nodeAnnotations(ctx, t, env, "n1")[racerctrl.NodeIDAnnotation]; id != "7" {
		t.Fatalf("a frozen pass retired node 7 anyway: id is %q", id)
	}

	if labels := nodeLabels(ctx, t, env, "n1"); labels[WorkloadLabel] != "true" {
		t.Fatalf("a frozen pass took the workload off a node holding data: %v", labels)
	}
}

// announce is the node agent saying it is up, which is the one racer annotation
// a node writes before it has an identity.
func announce(ctx context.Context, t *testing.T, env *component.Env, name string) {
	t.Helper()

	node := &corev1.Node{}
	if err := env.Client.Get(ctx, client.ObjectKey{Name: name}, node); err != nil {
		t.Fatalf("get node %s: %v", name, err)
	}
	...
	node.Annotations[racerctrl.NodeAgentAnnotation] = racerctrl.NodeAgentRunning

	if err := env.Client.Update(ctx, node); err != nil {
		t.Fatalf("announce %s: %v", name, err)
	}
}

// nodeAnnotations reads a node's annotations back out of the cluster.
func nodeAnnotations(ctx context.Context, t *testing.T, env *component.Env, name string) map[string]string {
	t.Helper()
...
// The DaemonSet must select on the operator-owned label rather than on site
// membership. This reads the template rather than the rendered output because
...
func TestDaemonSetSelectsTheWorkloadLabel(t *testing.T) {
	...
	if strings.Contains(manifest, component.SiteLabelKey+":") {
		t.Fatalf("the daemonset selects on %s, so a decommissioning node would lose its pod", component.SiteLabelKey)
	}
}
...
// identities, of which the first three belong in the zone and the rest are
// opted out. A node's cohort follows its id, because a cohort is frozen for a
// node's life.
...
// widen opts nodes in or out so the zone belongs to exactly width nodes.
...
func (z *scalingZone) settle() racerctrl.Catalog {
	...
	for range passes {
		...
		p, err := loadState(z.ctx, z.env, testEnrollment())
		if err != nil {
