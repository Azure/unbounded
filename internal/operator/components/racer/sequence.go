...
//
// Taking a node out of racer, by labelling it out or by turning racer off for
// its site, drops it out of every catalog's candidate set, which makes the
// membership sequencer step it out one universe at a time. Only when the node
// reports that it has finished shedding and holds no live pages is it safe to
// forget who it was; doing it earlier would strand whatever it had not handed
...
func (p *pass) retireDecommissionedNodes(ctx context.Context) error {
	for i := range p.nodes {
		...
		if view.eligible || view.state.ID == 0 {
			continue
		}
