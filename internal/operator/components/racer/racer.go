...
const (
	...
	// EnrollmentLabel keeps a node out of racer. Membership is otherwise
	// automatic: a node in a site that enables racer runs the racer pod, and
	// running racer is what makes it part of the cluster.
	//
	// Only the exact value "false" excludes, and nothing else about the label
	// means anything, so the label left over from when enrolment was explicit
	// ("true") is inert and can be deleted at leisure. Setting it to "false"
	// asks the node to leave, which is the same request as its site turning
	// racer off: the node is dropped from the candidate set and decommissioned
	// through the drain sequence rather than switched off.
	EnrollmentLabel = "racer.unbounded-cloud.io/enabled"

	// WorkloadLabel is what the DaemonSet selects on, and this component is its
	// only writer.
	//
	// It exists because "racer belongs on this node" and "racer should be
	// running on this node" are not the same thing. Taking a node out of racer,
	// by labelling it or by turning racer off for its site, asks for a
	// decommission, and R6's decommission needs the node to keep serving the
	// configs that step it out of each catalog, shed the groups it holds, and
	// report that it holds nothing. Selecting the pod on anything that changed
	// the moment the request was made would delete the pod there and then,
	// stranding whatever the node had not handed over. So the operator adds
	// this label to every node racer belongs on or that still holds an
	// identity, and takes it off only once the node has been retired.
...
func (Component) Reconcile(ctx context.Context, env *component.Env, sites []unboundedv1alpha3.Site) component.Result {
	enroll := enrollment{sites: make(map[string]bool, len(sites))}

	for i := range sites {
		if EnabledFor(&sites[i]) {
			enroll.sites[sites[i].Name] = true
		}
	}

	if len(enroll.sites) == 0 {
	...
	pass, err := loadState(ctx, env, enroll)
	if err != nil {
		return component.Failed(err)
	}
