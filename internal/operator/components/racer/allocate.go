...
// allocateNodeIdentities gives every node racer runs on the three values it
// needs before it can serve anything: an id, a zone, and a cohort.
//
...
// node twice.
//
// A node is placed once racer belongs on it and its agent has announced. The
// announcement is the point: it is written by a process that starts only after
// the preflight init container has claimed the store file, so waiting for it
// means a node whose disk is missing is never put into a catalog it cannot
// serve. Nothing rushes it, either. An unannounced node costs the zone nothing
// until it is placed.
...
// holds an identity, whether or not racer still belongs on it. A node on its way
// out still occupies its cohort until the catalog has let go of it.
...
// unplacedNodes is every node waiting for an identity. It is what decides
// whether a fabric has enough nodes waiting to be worth a zone of its own, so it
// has to be the whole batch rather than the one node in hand.
...
// candidates lists the nodes that belong in a zone: racer belongs on them, and
// they were given an identity in it.
//
...
func (p *pass) candidates(zone uint32) racerctrl.Membership {
	...
	for _, view := range p.nodes {
		if !view.eligible || view.state.ID == 0 || view.state.Zone != zone {
			continue
		}
