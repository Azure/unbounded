...
// coordination RPCs.
//
// Design (detailed-design.md §4.4, architecture.md §API):
//
...
// application/octet-stream — per plan §"Descriptor and media-type
...
func (s *Server) serveDigest(w http.ResponseWriter, r *http.Request, d digest.Digest, kind ifaces.OriginRefKind) {
	...
	if err != nil {
		...
		if errors.As(err, &eun) {
			// Plan §Phase 6 + §Phase 3: containerd unavailable is
