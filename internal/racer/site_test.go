...
func TestNodeMembership(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "deprecated", labels: map[string]string{racer.DeprecatedSiteLabelKey: "site-b"}},
		...
		{name: "excluded deprecated", labels: map[string]string{racer.DeprecatedSiteLabelKey: "site-b", racer.ExcludeLabelKey: "true"}},
...
func TestValidateBootstrapNode(t *testing.T) {
	for _, tc := range []struct {
		...
		{"fallback", map[string]string{racer.DeprecatedSiteLabelKey: "site-a"}, "uid", "site-a", false},
