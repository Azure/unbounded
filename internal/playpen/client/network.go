...
func (t *tunnel) setupCommands(privateKeyFile, endpoint string, serverWG, clientWG netip.Addr) ([][]string, error) {
	...
	guestGatewayPrefix, err := guestGatewayPrefix(t.metadata.Network)
	if err != nil {
		return nil, err
	}
	...
	commands := [][]string{
		...
		{"ip", "-n", t.cfg.NetworkNamespace, "addr", "add", guestGatewayPrefix, "dev", t.cfg.VXLANInterface},
	}
...
func guestGatewayPrefix(network operator.NetworkResponse) (string, error) {
	...
	return netip.PrefixFrom(gatewayIP, ones).String(), nil
}
