...
func AssignFabricDeviceID(self *NodeState, universeID uint32) (uint32, bool, error) {
	if universeID == 0 {
...
func AssignDeviceID(self *NodeState, volume string) (uint32, bool, error) {
	if volume == "" {
...
func lowestFreeMinor(self *NodeState) (uint32, error) {
	...
	for id := uint32(MinDeviceID); id <= MaxExports; id++ {
		if _, ok := taken[id]; !ok {
			return id, nil
		}
	}

	return 0, fmt.Errorf("node has no free device id; all %d exports are in use", MaxExports)
}
