...
type NodeState struct {
	...
	// Agent is non-empty once the node's agent has announced itself. It is what
	// enrols the node: the operator allocates an identity for a node in a
	// racer-enabled site only after the agent has said racer is running there.
	Agent string
