...
type Placer struct {
	...
	// unplaced counts the enrolled nodes still waiting for an identity, keyed by
	// site and fabric. It is what decides whether a fabric is worth a zone of
	// its own, and it is decremented as those nodes are placed.
...
// the node and enrol it again, which is a deliberate act with a known cost.
//
