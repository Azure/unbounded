...
const (
	// NodeAgentAnnotation is what enrols a node. The agent writes it as soon as
	// it starts, before it has an identity and before it has anything else to
	// say, and its presence is the node saying "racer is running here".
	//
	// It exists because the alternative signals are all worse. A label a human
	// sets is a second thing to remember and a source of drift. Pod presence
	// would make the operator watch Pods and would still not say whether the
	// dataplane could actually start. This one is written by the process that
	// runs only after the preflight init container has claimed the store file,
	// so it means the node is capable of holding data rather than merely
	// selected to.
	//
	// Identity allocation waits for it. A node whose disk is missing is
	// therefore never placed into a catalog, instead of being placed and then
	// never reporting.
	NodeAgentAnnotation = AnnotationDomain + "agent"
...
// NodeAgentRunning is the value NodeAgentAnnotation carries. The annotation is
// a presence signal, so readers test it for emptiness rather than for this
// exact string; it is spelled out so the Node object reads as a sentence rather
// than as a bare "true".
const NodeAgentRunning = "running"
