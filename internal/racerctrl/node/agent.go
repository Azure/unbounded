...
func (a *Agent) Reconcile(ctx context.Context) error {
	// Announcing comes first, and in particular before the identity check
	// below. It is what asks for an identity, so an agent that waited until it
	// had one would wait forever.
...
// announce says that racer is running on this node.
//
// This is what enrols the node. A node in a site that enables racer gets the
// DaemonSet, the DaemonSet runs the preflight init container, preflight claims
// the store file, and only then does this process start; so the annotation
// means the node is able to hold data rather than merely selected to. The
// operator allocates an identity for nodes that have it and for no others,
// which is why it is written before FindSelf rather than alongside the rest of
// the status: the status is published only once there is an identity, and the
// identity is what this is asking for.
//
// Presence is the whole signal, so a node that has already announced is left
// alone. The agent never withdraws it: an agent that has stopped cannot write,
// and a node that stops running racer is decommissioned by the operator through
// the drain sequence, not by an annotation disappearing.
func (a *Agent) announce(ctx context.Context) error {
	...
	if node.Annotations[racerctrl.NodeAgentAnnotation] != "" {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("build node announce patch: %w", err)
	}
	...
	a.log.Info("announced racer agent", "node", a.cfg.NodeName)
