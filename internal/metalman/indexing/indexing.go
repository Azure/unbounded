...
const IndexNodeByMAC = ".spec.host.netboot.dhcpLeases.mac"
...
func IndexNodeByMACFunc(obj client.Object) []string {
	...
	macs := make([]string, 0, len(node.Spec.Netboot().DHCPLeases))
	for _, lease := range node.Spec.Netboot().DHCPLeases {
...
const IndexNodeByIP = ".spec.host.netboot.dhcpLeases.ipv4"
...
func IndexNodeByIPFunc(obj client.Object) []string {
	...
	ips := make([]string, 0, len(node.Spec.Netboot().DHCPLeases))
	for _, lease := range node.Spec.Netboot().DHCPLeases {
		if lease.IPv4 != "" {
