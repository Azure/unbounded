...
- To build Racer, install Rust 1.96.0, `cc`, `ar`, `make`, Perl, and libibverbs development headers (Ubuntu: `build-essential perl libibverbs-dev`), then run `make racer-build`. This produces `bin/racer-controlplane`, `bin/racer-dataplane`, and `bin/racer-loadgen`. Rust uses its committed Cargo.lock, statically linked vendored OpenSSL with kTLS enabled, and vendored protoc with `../../api/racer`; no system OpenSSL, protoc, or liburing library is needed for this crate. Runtime kTLS eligibility requires Linux >= 6.14. To select an external OpenSSL instead, set `OPENSSL_NO_VENDOR=1` and install its development headers and `pkg-config` (or set `OPENSSL_DIR`); both the Rust bindings and C shim use that installation. External OpenSSL must be built with kTLS enabled and be >= 3.5 for offload; older libraries use encrypted software TLS for the whole connection. Set `RACER_REQUIRE_KTLS=1` when testing on a capable host to require actual TX/RX offload.
