...
- To build Racer, install Rust 1.96.0, `cc`, `ar`, `make`, Perl, libibverbs and OpenSSL development headers, and `pkg-config` (Ubuntu: `build-essential perl libibverbs-dev libssl-dev pkg-config`), then run `make racer-build`. This produces `bin/racer-controlplane`, `bin/racer-dataplane`, and `bin/racer-loadgen`. Both Rust crates use their own committed Cargo.lock and vendored protoc with `../../api/racer`. The control plane uses system OpenSSL for certificate cryptography and rustls for transport; its image builds with `libssl-dev` and runs with `libssl3t64`. Override `RACER_CONTROLPLANE_CARGO_TARGET_DIR` for isolated control-plane builds; `RACER_CARGO_TARGET_DIR` selects only the dataplane cache. Build recipes pass `VERSION`, `GIT_COMMIT`, and `BUILD_TIME`; the control plane also receives `RACER_VERSION` and `RACER_COMMIT` for its native version output.
- The dataplane statically links vendored OpenSSL with kTLS enabled; it needs no system OpenSSL, protoc, or liburing library. Linux >= 5.15 is the minimum kTLS baseline, not proof of runtime capability: every dataplane TLS session, including bootstrap, requires actual TX and RX kTLS before application admission. Only TLS 1.3 with AES-128-GCM or AES-256-GCM is supported; there is no software TLS fallback or compatibility mode. To select an external OpenSSL, set `OPENSSL_NO_VENDOR=1` and install its development headers and `pkg-config` (or also set `OPENSSL_DIR`); both the Rust bindings and C shim use that installation. External OpenSSL must be >= 3.5 and built with `enable-ktls`; older or kTLS-disabled builds are rejected. Supported kernel KeyUpdate continues on kTLS; unsupported or failed key replacement permanently fails the session, and recovery requires a fresh connection.
- TLS tests always require real TX/RX kTLS; there is no optional offload assertion toggle. After `make racer-rust-test-compile`, run `make racer-ktls-test` for strict admission, cipher negotiation, HTTP/sendfile, and rekey/reconnect coverage. This target also requires usable io_uring and thread-local seccomp filters for socket-scoped key-installation fault tests. CI records its actual host kernel; fault injection covers rejected key installation/replacement but does not establish runtime coverage on older kernels.
...
- `make e2e-racer-build` builds the four smoke-test images; `make e2e-racer` runs the single Racer kind smoke test with a five-minute timeout: one production control plane and dataplane, installed by the operator, with Go SDK cold/warm/range verification. It requires Docker, kind, kubectl, usable io_uring, sufficient physical cores/memory, and ext4 scratch. Set `RACER_E2E_DIR` to select scratch and `RACER_E2E_IMAGE_TAG` to select current-checkout images (default `racer-e2e`). The test cleans up its cluster, retains reusable images, and saves failure diagnostics. `make e2e-racer-compile` checks compilation without runtime prerequisites. CI runs this same smoke test; Go and Rust unit/component tests run separately.
