...
- To build Racer, install Rust 1.96.0, `cc`, `ar`, `pkg-config`, and libibverbs and OpenSSL 3 development headers (Ubuntu: `build-essential libibverbs-dev libssl-dev pkg-config`), then run `make racer-build`. This produces `bin/racer-controlplane`, `bin/racer-dataplane`, and `bin/racer-loadgen`. Rust uses its committed Cargo.lock and vendored protoc with `../../api/racer`; no system protoc or liburing library is needed for this crate. Runtime kTLS eligibility requires OpenSSL >= 3.5 and Linux >= 6.14; OpenSSL 3.0 uses encrypted software TLS for the whole connection.
...
- `make racer-crosslang-test` builds and selects the exact Rust lib-test executable and enables both `RACER_COORDINATION_TEST_BIN` and `RACER_DATAPLANE_BINARY` for the Go coordination/SDK and live CA-retirement suites. Set `TMPDIR` to workspace-local ext4 scratch space and provide sufficient locked memory and physical cores. Set `KUBEBUILDER_ASSETS` separately to enable API-server integration tests.
