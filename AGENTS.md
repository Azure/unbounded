...
- `make e2e-gantry-racer-build` builds Gantry, the load generator, and the debug Rust dataplane and warms the Go e2e build cache. `make e2e-gantry-racer` runs the six native Gantry/Racer/containerd suites (including explicit generation recovery) with prebuilt binaries and independent external deadlines. It requires workspace-local ext4 scratch, passwordless sudo, containerd, private network/mount namespaces, and real io_uring support. CI runs this integration alongside the native control-plane campaign and SDK tests.
