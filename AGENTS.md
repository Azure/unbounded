...
- `make racer-crosslang-test` builds the daemon for Go SDK interoperability tests. `make racer-controlplane-live-test` builds both Rust binaries and runs the `e2e`-tagged production-binary campaign against a real API server: two control planes, three dataplanes, independent convergence, failover, storage resize/restart, and CA rotation through root retirement with continuous verified reads. Set `KUBEBUILDER_ASSETS`, workspace-local ext4 `TMPDIR`, and an existing workspace-local `RACER_LIVE_SOCKET_ROOT` whose absolute path is at most 29 bytes. Provide sufficient locked memory and physical cores plus passwordless sudo for private network/mount namespaces. Missing prerequisites fail the explicit live target. `make e2e-racer-compile` compiles the campaign without requiring its runtime prerequisites.
- `make e2e-gantry-racer-build` builds Gantry, the load generator, and the debug Rust dataplane and warms the Go e2e build cache. `make e2e-gantry-racer` runs the seven native Gantry/Racer/containerd suites (including explicit generation recovery and rolling backend switches) with prebuilt binaries and independent external deadlines. It requires workspace-local ext4 scratch, passwordless sudo, containerd, private network/mount namespaces, and real io_uring support. CI runs this integration alongside the native control-plane campaign and SDK tests.
