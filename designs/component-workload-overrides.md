...
unbounded-operator generates and reconciles the workloads for these components:
`net`, `machina`, `gantry` (cluster singletons) and `metalman`
...
manager `unbounded-operator` and `client.ForceOwnership`. The YAML-driven
...
**3.6 Two different generation paths.** Several components decode embedded YAML into
...
to "produce objects, apply them". The existing components perform several
...
| `component` | yes | One of `net`, `machina`, `gantry`, `metalman`. Matched against `ClusterComponent.Name()` / `SiteComponent.Name()`. |
...
to reconstruct derived names such as `metalman-controller-<site>`.
...
- For `metalman`: one object per matched Site, named by the
  component's own derivation (`metalman.go:100`).
...
mandatory Site affinity that `metalman` depends on. See
...
a separate operation in the same plan. Several components stamp a hash
...
applying anything, and net writes a ConfigMap before the DaemonSet that
...
- Within-component operation order has to be preserved rather than sorted, or
  gantry's legacy cleanup and net's ConfigMap-before-DaemonSet sequencing
