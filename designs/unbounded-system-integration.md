...
0. **Translate Sites.** For each pre-redesign net-group Site
   ...
   `spec.components` inferred from the running legacy workloads: machina is
   enabled on the `cluster` Site, and metalman where a
...
per-site metalman and the net singleton) before the reaper drains the old namespaces; the per-component
