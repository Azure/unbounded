...
Unbounded installs its components across several namespaces: machina and metalman
land in `unbounded-kube`, unbounded-net lands in
...
|----|-------|--------------|--------|------------|
| **PR-B** | Namespace unification: `UNBOUNDED_NAMESPACE`, `internal/unbounded`, template defaults across machina/net/gantry/inventory/orca/machine-ops, `migrate-namespace.sh`, smoke test | ~194 | existing open PR #372 (`unify-namespace-unbounded-system`), re-targeted to this branch | - |
...
0. **Translate Sites.** For each pre-redesign net-group Site
   ...
   `spec.components` inferred from the running legacy workloads: machina on the
   `cluster` Site, and metalman where a
...
1. Copy non-regenerable state (operator/user Secrets and the `machina-config`
   ...
   and is then preserved once the source namespace is drained.
...
per-site metalman and the net singleton) before the reaper drains the old
namespaces; the per-component
...
- **In-process simulation** (`e2e/operator`, Go, `//go:build e2e`, per-PR): drives
  ...
  legacy workloads and Ready targets. Fast; covers the reaper logic including
   metalman reaping and the per-site readiness gate.
- **Faithful released-version upgrade** (`hack/operator-upgrade-e2e/e2e.py`,
  ...
  upgrades via this tree's `kubectl unbounded install` and asserts the operator's
   reaper migrates net + machina for real. Metalman (PXE)
   cannot run in vanilla kind, so it is excluded from this path and stays
...
- `net.unbounded-cloud.io/site` is deprecated in favor of
  `unbounded-cloud.io/site`. During the deprecation window net dual-writes both
   labels and metalman targets either key so it can schedule before the
